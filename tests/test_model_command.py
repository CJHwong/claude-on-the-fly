...
def test_parse_refuses_effort_on_pty_and_changes_nothing():
    result = mc.parse(["opus", "high"], _profile(mode="pty"))

    assert isinstance(result, mc.Refusal)
    assert "pty" in result.text
...
def test_codex_merges_both_cache_files(tmp_path, monkeypatch):
    home = tmp_path / "codex-home"
    home.mkdir()
    ...
    assert mc._codex_catalogue().models == ["gpt-5.6-sol", "gpt-pinned"]
...
def test_describe_omits_the_effort_line_on_pty(claude_catalogue):
    text = mc.describe(_profile(mode="pty"), pinned=False)

    assert "not used in pty mode" in text
    assert "Effort for" not in text
