...
from claude_on_the_fly.symphony.tracker.issue import make_key

_JIRA_KEY = make_key("jira", "id1")
...
def test_schedule_failure_increments_attempt_explicitly():
    ...
    assert q.get_attempt(_JIRA_KEY) == 1
    q.schedule_failure("id1", "PROJ-1", max_backoff_ms=300_000, attempt=2)
    assert q.get_attempt(_JIRA_KEY) == 2
...
def test_due_now_returns_and_removes_due_entries():
    ...
    assert not q.has(_JIRA_KEY)  # removed
...
def test_continuation_with_no_prior_starts_at_zero():
    q = RetryQueue()
    q.schedule_continuation("id1", "PROJ-1")
    assert q.get_attempt(_JIRA_KEY) == 0
...
def test_schedule_with_explicit_source_uses_composite_key():
    """Same issue_id in two sources scheduled independently."""
    q = RetryQueue()
    q.schedule_continuation("42", "PROJ-42", source="jira")
    q.schedule_continuation("42", "owner/repo#42", source="github")
    assert q.has(make_key("jira", "42"))
    assert q.has(make_key("github", "42"))
    assert len(q.all_pending()) == 2
