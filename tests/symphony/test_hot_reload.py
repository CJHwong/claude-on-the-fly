...
# ---------------------------------------------------------------------------
# _read_config_refresh_ms — wires config_refresh_ms to the refresh timer
# ---------------------------------------------------------------------------


def test_read_config_refresh_ms_none_without_config_source(tmp_path: Path) -> None:
    from claude_on_the_fly.symphony.orchestrator import _read_config_refresh_ms

    cfg = tmp_path / "symphony.yaml"
    cfg.write_text("polling_ms: 30000\ntracker:\n  base_url: x\n  project_key: P\n")
    assert _read_config_refresh_ms(cfg) is None


def test_read_config_refresh_ms_reads_value(tmp_path: Path) -> None:
    from claude_on_the_fly.symphony.orchestrator import _read_config_refresh_ms

    cfg = tmp_path / "symphony.yaml"
    cfg.write_text(
        "config_source: git+https://example/repo@main\nconfig_refresh_ms: 600000\n"
    )
    assert _read_config_refresh_ms(cfg) == 600000


def test_read_config_refresh_ms_floors_at_30s(tmp_path: Path) -> None:
    from claude_on_the_fly.symphony.orchestrator import _read_config_refresh_ms
    ...
    assert _read_config_refresh_ms(cfg) == 30000


def test_read_config_refresh_ms_defaults_when_absent(tmp_path: Path) -> None:
    from claude_on_the_fly.symphony.orchestrator import _read_config_refresh_ms

    cfg = tmp_path / "symphony.yaml"
    cfg.write_text("config_source: git+https://example/repo@main\n")
    assert _read_config_refresh_ms(cfg) == 300000


def test_force_reload_bypasses_mtime_gate(tmp_path: Path) -> None:
    """force=True must reload even when the local mtime is unchanged — that's
    how the periodic remote refresh picks up upstream-only changes."""
    from unittest.mock import MagicMock

    from claude_on_the_fly.symphony.config import load_config
    from claude_on_the_fly.symphony.orchestrator import _maybe_reload_config
...
""",
    ...
    # Same mtime + force=True → still reloads (returns a fresh config object).
    new_cfg, new_mtime = _maybe_reload_config(
        config_path=cfg_path,
        config=cfg,
        last_mtime=mtime,
    ...
    assert new_cfg is not cfg  # reloaded despite unchanged mtime
    assert new_mtime == mtime
