...
**codex under the jail had no credential handoff.** The daemon hands claude its token
...
identity JWT payload is not valid JSON"). So a jailed codex read `auth.json`, refresh
token included.

`agent.codex.chatgpt_via_broker` closes it with a broker route; `docs/agent/broker.md`
has the design and the measurements. Still open:

- **Stability.** `/backend-api/` is not a public API. A codex release can change its
  paths or headers, so each upgrade needs the route re-run.
