...
- **Text formatting**: the agent writes plain Markdown on every platform (`FORMAT_HINTS` in `agent.py`), and each frontend renders it. Slack has two surfaces with different parsers, and picking the wrong one is the whole bug class here. A reply body goes in a `markdown` block, which Slack parses server-side into native `rich_text_list` (with real indent) and native `table` blocks; feed it the Markdown as written and convert nothing. Everything else on Slack is a `section` or `context` block, which take legacy `mrkdwn` only — no lists, no tables — so text bound for one of those goes through `slack_mrkdwn.to_mrkdwn()` first. The mid-turn progress line is the one place agent text still takes that path, because it lands in a `context`. Never hand-build `rich_text` blocks, and never put mrkdwn's `*bold*` or a literal `•` into a `markdown` block — both render as literal text.
- **Reply length**: two Slack caps apply, and the message cap is the smaller one. A `markdown` block holds 12000 characters (characters, not bytes: 12000 CJK characters pass), and a whole message holds less than that, though Slack publishes no number for it: `msg_blocks_too_long` arrives with no count. `slack_mrkdwn.split_blocks()` cuts the body at `SLACK_MESSAGE_CHAR_LIMIT` and `slack._reply_messages()` posts one message per chunk, so a long reply is several messages in the thread. The stats/tools footer and the suggestion buttons ride the last message only. The split prefers a blank line, then a line boundary, and never lands inside a table or leaves a fence open; a table or fence longer than the limit is the one case that gets cut mid-way. When a part still fails, `_notify_send_failure` posts the error code in-thread, because a reply that never arrives otherwise reads as the agent ignoring you.
