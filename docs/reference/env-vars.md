...
| `SLACK_JOB_COMMAND` | no | Text prefix that queues a background job, e.g. `$job`. A message starting with it is handed to the `claude-jobs` worker, which runs the rest of the message in a fresh session and replies in the same thread when it finishes — so the task outlives the chat turn that asked for it. Unset (the default) registers no trigger at all and such a message is answered as ordinary text. Works under either token kind, and inside threads. Pick a punctuation-led value: the trigger is matched against the head of every message, so a plain word swallows every message beginning with it. If `SLACK_TOKEN` is a user token, also set `JOBS_SLACK_TOKEN` to a bot token — otherwise the worker's replies post as you and this daemon re-ingests them as new input |
